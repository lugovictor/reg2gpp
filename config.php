<?php
#    Copyright 2013 Sean Mackedie
#
#
#    This file is part of Reg2GPP.
#
#    Reg2GPP is free software: you can redistribute it and/or modify
#    it under the terms of the GNU Affero General Public License as published by
#    the Free Software Foundation, either version 3 of the License, or
#    (at your option) any later version.
#
#    Reg2GPP is distributed in the hope that it will be useful,
#    but WITHOUT ANY WARRANTY; without even the implied warranty of
#    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
#    GNU Affero General Public License for more details.
#
#    You should have received a copy of the GNU Affero General Public License
#    along with Reg2GPP.  If not, see <http://www.gnu.org/licenses/>.

# Set the base directory for the tool. Leave blank for relative current path
define("BASEDIR", "", true);

# Set the webmaster's email for bug reporting
define("WEBMASTER_EMAIL", "webmaster@example.com");

# Set the form submit files, useful e.g. if embedding the tool in a CMS like Joomla
# By default, UPLOAD_FILE should be 'upload.php' and DOWNLOAD_FILE should be 'download.php'
define("UPLOAD_FILE", "upload.php");
define("DOWNLOAD_FILE", "download.php");